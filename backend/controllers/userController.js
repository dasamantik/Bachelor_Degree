...
export const register = async (request, reply) => {
  try {
    ...
    if (error) return reply.status(400).send({ message: `Помилка валідації: ${error.details[0].message}` });
    const { email, name, phone, password } = request.body;
    const userData = await userService.userRegistration(email, name, phone, password);
